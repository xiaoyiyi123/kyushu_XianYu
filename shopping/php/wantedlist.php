...
if($parameter==0){
    ...
    for($i=0;$i<$nums;$i++){
        ...
        array_push($arr,array('purchase_Id'=>$row["purchase_Id"],'goods'=>$row["goods"],'budget'=>$row["budget"],'note'=>$row["note"],'category'=>$row["category"],'place'=>$row["place"],'picutre'=>$row["picure"]));
    }
    $msg=array
    ( 
      "Num"=>$nums,
