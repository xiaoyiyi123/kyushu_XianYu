...
if($parameter==0){
    ...
    //query from mysql
    $sql = "SELECT * FROM Purchase where purchase_Id ='".$item_id."'";
    //$result = $conn->query($sql);
    $result = $conn->query($sql);
    $row = $result->fetch_assoc();
    $msg=array('Name'=>$row['goods'],'Price'=>$row['budget'],'Picture'=>$row['img'],
    'smallPicture1'=>$row['img'],'smallPicture2'=>$row['img'],'smallPicture3'=>$row['img'],'campus'=>$row['place'],'note'=>$row['note'],'userName'=>$row['user_Name'],'category'=>$row['category']
    );
    $item_Name = $row['goods'];
